...
| `--force` | пересобрать и перезапустить всё, не сверяясь с диффом |
| `--help` | справка |

Если `git pull` обновил сам `deploy.sh`, скрипт перезапускается на новой версии
(бэкап и уже сделанный pull не повторяются). Без этого bash доигрывал бы смесь старой
и новой версии: он читает файл по мере выполнения, а не целиком.

Что уже выкачено, скрипт помнит в `~/backups/subscription-tracker/last-deployed-commit`
и сверяет дифф именно с ним, а не с предыдущим `HEAD`. Это важно: если статика отстала
от кода (например, после ручного или прерванного деплоя), сравнение с `HEAD` дало бы
«фронт не менялся» и оставило старый бандл. Файла нет — выкатывается всё целиком.
