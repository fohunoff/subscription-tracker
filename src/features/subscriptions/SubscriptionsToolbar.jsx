...
function SubscriptionsToolbar({ searchQuery, onSearchChange, viewMode, onViewModeChange, foundCount }) {
  const buttonBase =
    'flex flex-1 sm:flex-none items-center justify-center gap-1.5 px-3 py-2.5 sm:py-2 text-sm font-medium rounded-md transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-brand-primary focus:ring-opacity-50';
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between mb-6">
      {/* text-base у поля поиска на мобильных: с меньшим шрифтом Safari на iOS
          зумит страницу при фокусе, и список уезжал за край экрана */}
      <div className="relative flex-1 max-w-md">
        ...
        <input
          ...
          className="block w-full text-base sm:text-sm rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 pl-10 pr-9 py-2.5 sm:py-2 shadow-sm focus:border-brand-primary focus:outline-none focus:ring focus:ring-brand-primary focus:ring-opacity-90"
        />
        {searchQuery && (
          <button
            type="button"
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-1 sm:inline-flex sm:flex-none bg-slate-100 dark:bg-slate-700/50 rounded-lg p-1" role="group" aria-label="Вид отображения">
          <button
            type="button"
