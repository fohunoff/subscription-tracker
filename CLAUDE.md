...
автозапись не выдумывает — их достраивает бэкфилл (ниже), который умеет читать
...
Прошлое достраивает `server/services/paymentBackfill.js`. Даты берёт по циклу
от даты старта, цену — из лога изменений (значение на дату платежа — это `from`
ближайшего последующего изменения), промежутки в архиве пропускает по событиям
`archived` → `returned`/`restored`.

Вызывается он **на каждое создание подписки** (`POST /subscriptions` и импорт),
а не только скриптом: «плачу за это с 2015 года» — обычный случай, дата старта
в форме на то и есть. Пока бэкфилл был разовой миграцией, подписка, заведённая
после её прогона, появлялась в тратах только с сегодняшнего дня, и раздел
«Сколько уже потрачено» начинался с даты, не значившей ничего, — история
молча теряла годы. Ответ несёт `estimatedPayments`, клиент говорит об этом
в тосте: сумма трат выросла не сама по себе.

Правка `fullPaymentDate` или `cycle` в `PUT /subscriptions/:id` пересобирает
оценки заново — тот же `syncEstimatedPayments`, но с `reset`: даты прошлых
платежей считаются от даты старта шагами цикла, и после правки прежняя история
описывала бы уже не эту подписку. Ответ несёт `recalculatedPayments`.

Скрипт `server/scripts/backfill-payments.js` остался для разовых прогонов по
всей базе (без флага показывает, `--apply` применяет, `--reset` пересобирает
заново) — им же чинят данные, накопленные до автоматического бэкфилла.
Подписки, по которым добавить нечего, он печатает с датой старта и границей
учёта: пустая история почти всегда объясняется исходными данными, а не
расчётом.

Восстановленные записи помечены `estimated: true`: достоверность у них ниже —
лог ведётся не с начала времён. Пометка же позволяет пересобрать оценки,
не тронув наблюдённые платежи. Событие получает
...
Период «за всё время» (`?all=1`) — единственный, где нижнюю границу выбирает
сервер, а не клиент: он берёт дату первого события `payment` в логе. Клиент не
может подставить её сам, не угадывая глубину истории, а запас «лет на десять»
дал бы график из пустых столбцов. Ответ несёт в `from` именно эту дату, и
интерфейс подписывает ею сумму («За всё время, с февраля 2024»). Если платежей
нет вовсе, период сводится к текущему месяцу.
...
столбца, сегменты разделяет зазор цвета поверхности. Отступ под ось (`padLeft`)
считается по самой широкой подписи, а не задан константой: её ширина зависит от
разрядности суммы и длины кода валюты, и с жёстким значением «150 тыс. RSD»
обрезалась левым краем viewBox до «50 тыс. RSD» — верх шкалы читался втрое
...
**Мобильная вёрстка — решения, которые легко откатить по невнимательности.**
Проверять правки нужно минимум на 320, 390 и 768 px: часть ошибок видна только
на одной из этих ширин. Помогает `scripts/layout-audit.js` — проверка без
зависимостей, вставляется в консоль браузера или в `browser_evaluate`; ищет
переполнение вьюпорта, кнопки поверх заголовков, налезающий друг на друга текст,
тап-цели меньше 44px и поля со шрифтом меньше 16px.

1. **Кнопки темы, настроек и профиля до `lg` живут в липкой полосе**, а не висят
   в углу (`sticky` → `lg:fixed` в `App.jsx`). Плавающими они перекрывали
   заголовок страницы (на 390px «Трекер расходов» пропадал под ними целиком) и
   заголовки секций при прокрутке. Порог именно `lg`: на планшете в портрете
   кнопка темы ещё задевала последнюю букву заголовка.
2. **Кнопки управления категорией рендерятся дважды** (`controlButtons` в
   `CategorySection`): с `sm` — абсолютом в правом верхнем углу, до `sm` — в
   строке с итогами рядом с «Добавить». Абсолютные иконки накрывали название
   категории, а отступ справа сжимал заголовок до нечитаемой ширины.
3. **Поля ввода на мобильных — `text-base` (16px)**, а не `text-sm`. Safari на
   iOS зумит страницу при фокусе в поле с меньшим шрифтом, и после ввода форма
   оказывается за краем экрана. Это правило касается и поиска в
   `SubscriptionsToolbar`, и обеих форм.
4. **Календарь `react-datepicker` на узких экранах открывается через
   `withPortal`** (`useMediaQuery` в `SubscriptionForm`). Всплывающий календарь
   лежит внутри прокручиваемой формы, и `popperPlacement="auto"` его не
   переворачивал: на 320×720 сетка дней целиком уходила ниже экрана.
5. **У графика трат отдельная геометрия для узких экранов** (`VIEW_COMPACT` в
   `SpendingChart`) — viewBox сужается, а не масштабируется: 720 пикселей,
   сжатые до 340, уменьшали и подписи, 11px превращались в нечитаемые 5px.
   Там же подписи оси идут без символа валюты и месяц выбирается тапом:
   на тачскрине наведения нет, и строка со значением иначе оставалась пустой.
6. **Названия подписок — `line-clamp-2 sm:truncate`.** На мобильных обрезка
   съедала больше половины названия, а раскрыть его было негде.

Тёмная тема доделана не везде — `Modal` и карточка `SubscriptionItem` всё ещё
без `dark:`-классов (см. `TODO.md`).
