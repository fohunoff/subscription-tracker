...
экрана без прокрутки. У панели прокручивается только содержимое, заголовок
закреплён. `Modal` (формы) заодно получил `max-height` и внутреннюю прокрутку:
форма подписки тоже перестала влезать в экран.
